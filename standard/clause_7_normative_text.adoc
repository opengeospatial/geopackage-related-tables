...
A user-defined mapping table MAY be implemented as a GPKG http://www.geopackage.org/spec/#attributes[attributes] table type, and MAY contain other columns not listed here.
====
...
This requirements class allows a GeoPackage producer or editor to link metadata which may already exist in a GPKG Attributes table, including data types beyond the Simple Attributes set (e.g. GEOMETRY and BLOB).
